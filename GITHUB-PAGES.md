...
- A blank page or missing files: confirm `dist/index.html`, `app.js`, `engine.js`, `events.js`, `labels.js`, `drag.js`, `results.js`, and `style.css` are all present. The workflow must publish `dist`, not the entire repository.
